(function ($) {
    ...
    WeekDayPicker.prototype = {
        ...
        init: function () {
            ...
            var handler = $('<a href="javascript:;" class="weekdaypicker-handler">' + (editable ? "点击编辑" : "点击查看") + ' <i class="fa fa-calendar"></i></a>');
            parent.append(handler);

            var html = '<div class="weekdaypicker"><div class="weekdaypicker-head">';
            if (editable) {
                html += '<a href="javascript:;" data-filter="0,167">全部时间</a><a href="javascript:;" data-filter="0,119">周一至周五</a><a href="javascript:;" data-filter="120,167">周末</a>';
            }
            html += '<div class="info"><i class="full"></i> <span>投放时间段</span> <i class="stop"></i> <span>暂停时间段</span>';
            if (editable) {
                html += ' <span>（按住Ctrl键连选）</span>';
            }
            html += '</div></div><div class="weekdaypicker-body"></div><div class="weekdaypicker-foot">';
            if (editable) {
                html += '<button type="button" class="button ok">确定</button><button type="button" class="button cancel">取消</button>';
            } else {
                html += '<button type="button" class="button cancel">关闭</button>';
            }
            html += '</div></div>';
            var wrap = $(html).hide();
            parent.append(wrap);
            ...
            $.each(weeks, function (i, n) {
                ...
                if (editable) {
                    label.append('<input type="checkbox" /><i></i>' + n).addClass("i-check");
                } else {
                    label.append(n).addClass("week-text");
                }
                row.append(label);
