        ...
        for ($i=1; $i<2; $i++) {
            ...
            foreach ($stats  as $stat) {
                ...
                if ($consumption->get('consumable_type') =='App\Network') {
                    ...
                    $ideas_stats[$stat['idea_id']]['cost'] -= $consumption_daily->get('cost');
                    $ideas_stats[$stat['idea_id']]['consumption_total'] -= $consumption_daily->get('consumption_total');
                }
            ...
            foreach ($plans_stats as $plan_id =>$plan){
                if ($lastHour != 1){
                    Plan ::where ('id', $plan_id)->increment('daily_consume',  $plan['consumption_total']);
                }else{
                    Plan ::where ('id', $plan_id)->update(['daily_consume'=>$plan['consumption_total']]);
                }
            }
